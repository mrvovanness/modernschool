...
doc.xpath('//tr')[1..18].each do |x|
  Course.create(title: x.css('a')[0].text, description: x.css('p').text, user_id: 1)
end
...
14.times do |i|
  Lesson.create(title: titles[i+1], description: descriptions[i+1], course_id: 1)
end
...
news.each do |n|
  Post.create(user_id: 1, title: n.css('h3.list_item_title a').text, body_markdown: n.css('div.list_item_announce').text)
end
